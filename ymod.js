(function () {
   ...
   var countryNames = {
    ...
    var titleCache = Lampa.Storage.get("title_cache_hybrid_ru_v3") || {};
    var studiosCache = {};
    var uatorCache = {};
    ...
    var styles = `
        ...
        @media screen and (orientation: portrait), screen and (max-width: 767px) {
            ...
            .plugin-hybrid-title__body { justify-content: center !important; }
            .omdb-mdb-yarik-rate { justify-content: center; }
            .plugin-ru-title-combined { align-items: center !important; text-align: center !important; }
            .studio-logos-container { justify-content: center !important; }
            .quality-badges-container { width: 100%; justify-content: center; display: block !important; margin: 10px 0; clear: both; }
            .qb-unified-block { flex-wrap: wrap; justify-content: center; width: 100%; }
    ...
    function handleLogo(e) {
        ...
        var target_lang = user_lang ? user_lang : "ru";
        var size = Lampa.Storage.get("logo_size", "original");
        var cache_key = "logo_cache_ru_v3_" + type + "_" + data.id + "_" + target_lang;
        ...
        function startLogoAnimation(img_url, save_to_cache) {
            ...
            img.onload = function () {
                ...
                img.style.marginTop = "0"; img.style.marginBottom = "0";
        ...
        if (data.id) {
            ...
            $.get(url, function (data_api) {
                var final_logo = null;
                if (data_api.logos && data_api.logos.length > 0) {
                    var found = data_api.logos.find(function(l) { return l.iso_639_1 == target_lang; }) || data_api.logos.find(function(l) { return l.iso_639_1 == "ru"; }) || data_api.logos.find(function(l) { return l.iso_639_1 == "en"; });
                    if (found) final_logo = found.file_path;
                }
                if (final_logo) {
                    var img_url = Lampa.TMDB.image("/t/p/" + size + final_logo.replace(".svg", ".png"));
    ...
    function handleHybridTitle(e) {
        ...
        $.getJSON(url, function (data) {
            var hasRuLogo = false;
            if (data.images && data.images.logos) {
                hasRuLogo = data.images.logos.some(function (l) { return l.iso_639_1 === "ru"; });
            }
            var originalName = data.original_title || data.original_name || card.original_title || card.original_name || "";
            var enTitle = data.title || data.name || originalName;
            var ruTitle = enTitle;
            if (data.translations && data.translations.translations) {
                ...
                if (translation) ruTitle = translation.data.title || translation.data.name || enTitle;
            }
            var dateStr = data.release_date || data.first_air_date || "";
            var year = dateStr ? dateStr.split("-")[0] : "";
            ...
            titleCache[card.id] = { ruTitle: ruTitle || "", enTitle: enTitle || "", hasLogo: hasRuLogo, year: year || "", country: countryString || "", timestamp: now };
            Lampa.Storage.set("title_cache_hybrid_ru_v3", titleCache);
            renderHybridTitle(render, ruTitle, enTitle, hasRuLogo, year, countryString);
        }).fail(function() {
            var fallbackTitle = card.title || card.name || card.original_title || "";
            renderHybridTitle(render, fallbackTitle, fallbackTitle, false, "", "");
        });
    ...
    function renderStudiosTitle(render, title, movie) {
        if (!render) return;
        $(".plugin-ru-title-combined", render).remove();
        ...
        if (movie && movie.production_companies) {
            ...
            companies.forEach(function (co, index) {
                ...
                if (!showBg && index > 0) html += '<span style="color: rgba(255,255,255,0.4); margin: 0 ' + gapEm + '; font-size: 0.6em; display: inline-flex; align-items: center;">●</span>';
        ...
        var bgCSS = showBg ? 'background: rgba(255,255,255,0.08) !important; padding: 5px 12px !important; margin-right: ' + gapEm + ' !important;' : 'background: transparent !important; border: none !important; padding: 5px 0px !important; margin-bottom: 0.2em !important;';
        var wrap = $('<div class="plugin-ru-title-combined" style="margin-top: 10px; margin-bottom: 5px; text-align: left; width: 100%; display: flex; flex-direction: column; align-items: flex-start;"><div class="studio-logos-container" style="display: flex; align-items: center; flex-wrap: wrap;">' + html + '</div></div>');
    ...
    function handleStudios(e) {
        ...
        if (cached && (now - cached.timestamp < 180000)) {
            renderStudiosTitle(render, cached.ru_title, cached.full_data);
        } else {
            var type = card.first_air_date ? "tv" : "movie";
            Lampa.Api.sources.tmdb.get(type + "/" + card.id + "?append_to_response=translations", {}, function (data) {
                ...
                var ru = found ? (found.data.title || found.data.name) : (card.title || card.name);
                studiosCache[card.id] = { ru_title: ru, full_data: data, timestamp: now };
                renderStudiosTitle(render, ru, data);
    ...
    function getBestAndPopular(results, movie) {
        ...
        // Поиск русского дубляжа/озвучки (вместо украинского)
        ...
        results.forEach(function(item) {
            ...
            var hasRu = ruPattern.test(title);

            if (!hasRu && item.ffprobe && Array.isArray(item.ffprobe)) {
                hasRu = item.ffprobe.some(function(s) {
        ...
        if (ruResults.length === 0) return { ru: false };

        var best = ruResults.reduce(function(p, c) { 
        ...
        var popular = ruResults.reduce(function(p, c) { return (p.seedersCount > c.seedersCount) ? p : c; });
        var tech = { hdr: false, dv: false, audio: null };
        var maxChannels = 0;

        ruResults.forEach(function(item) {
            if (item.ffprobe) {
                item.ffprobe.forEach(function(s) { if (s.codec_type === 'audio' && s.channels) maxChannels = Math.max(maxChannels, parseInt(s.channels)); });
            }
        });
        if (maxChannels > 0) tech.audio = (maxChannels >= 6) ? '5.1' : '2.0';

        return { ru: true, bestRes: getResolutionLabel(best.detectedWidth), popRes: getResolutionLabel(popular.detectedWidth), popSeeds: popular.seedersCount, tech: tech };
    }
    ...
    function renderUator(container, data) {
        ...
        if (!data.ru) {
            block.append('<div class="quality-badge qb-not-found"><span class="qb-text-icon">RU</span><span class="qb-text">нет</span></div>');
        } else {
            var items =[ {i: uatorIcons.ua, t: data.bestRes, type: 'ru'}, {i: uatorIcons.top, t: data.popRes} ];
            items.forEach(function(it) {
                var iconHtml = it.type === 'ru' ? '<span class="qb-text-icon">RU</span>' : '<img src="'+it.i+'" class="qb-prefix-icon">';
                block.append('<div class="quality-badge">' + iconHtml + '<span class="qb-text">' + it.t + '</span></div>');
            });
        }
        container.append(block);
    }
    ...
    function processUatorCards() {
        ...
        $('.card:not(.qb-processed)').each(function() {
            ...
            if (movie && movie.id) {
                ...
                var key = movie.id + '_ru';
                if (uatorCache[key]) renderUator(card.find('.card__view'), uatorCache[key]);
                else {
    ...
    Lampa.Listener.follow('full', function(e) {
        if (e.type === 'complite' || e.type === 'ready') if (isEnabled('slogan')) ymodCleanSlogan();
    ...
    function createSettings() {
        ...
        Lampa.SettingsApi.addComponent({ component: 'ym_title', name: 'Доп. название' });
        Lampa.SettingsApi.addComponent({ component: 'ym_ratings', name: 'Рейтинги' });
        Lampa.SettingsApi.addComponent({ component: 'ym_ratings_select', name: 'Выбор источников' });
        Lampa.SettingsApi.addComponent({ component: 'ym_studios', name: 'Студии' });
        ...
        addToggle(MAIN_C, 'slogan', "Скрыть слоган", "Убрать короткие фразы под заголовком");
        
        addStatic(MAIN_C, "ym_logo_entry", "Логотипы (Smart)", "Настройка замены текста на лого", function() { Lampa.Settings.create('ym_logo'); });
        addToggle('ym_logo', 'logo', "Включить", "Показывать графические лого вместо текста");
        addSelect('ym_logo', "logo_lang", "Язык лого", "Приоритет языка", { ru: "Русский", en: "English" }, "ru");
        addStatic('ym_logo', 'ym_logo_back', "Назад", "", function() { Lampa.Settings.create(MAIN_C); });

        addStatic(MAIN_C, "ym_title_entry", "Доп. название", "Оригинал, год, страна", function() { Lampa.Settings.create('ym_title'); });
        addToggle('ym_title', 'hybrid', "Включить", "Отображать оригинальное название и инфо");
        addSelect('ym_title', "hybrid_title_size", "Размер", "", { 's': 'Маленький', 'm': 'Средний', 'l': 'Крупный' }, 'm');
        addStatic('ym_title', 'ym_title_back', "Назад", "", function() { Lampa.Settings.create(MAIN_C); });

        addStatic(MAIN_C, "ym_ratings_entry", "Рейтинги (OMDB)", "Настройка внешних оценок", function() { Lampa.Settings.create('ym_ratings'); });
        addToggle('ym_ratings', 'ratings', "Включить", "Показывать IMDb, RT, Metacritic");
        addSelect('ym_ratings', 'omdb_rating_saturation', 'Насыщенность', '', { '100%': 'Цветные', '0%': 'Ч/Б' }, '100%');
        addStatic('ym_ratings', 'ym_ratings_back', "Назад", "", function() { Lampa.Settings.create(MAIN_C); });

        addStatic(MAIN_C, "ym_studios_entry", "Студии", "Логотипы компаний-производителей", function() { Lampa.Settings.create('ym_studios'); });
        addToggle('ym_studios', 'studios', "Включить", "Показывать логотипы студий (Marvel, Warner и др.)");
        addStatic('ym_studios', 'ym_studios_back', "Назад", "", function() { Lampa.Settings.create(MAIN_C); });

        addStatic(MAIN_C, "ym_uator_entry", "RuTor", "Индикаторы наличия русской озвучки", function() { Lampa.Settings.create('ym_uator'); });
        addToggle('ym_uator', 'uator', "Включить", "Маркеры разрешения и качества на карточках");
        addStatic('ym_uator', 'ym_uator_back', "Назад", "", function() { Lampa.Settings.create(MAIN_C); });
    }
